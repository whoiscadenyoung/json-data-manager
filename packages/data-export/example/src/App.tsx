...
function ExportRow({ id }: { id: string }) {
  ...
  return (
    <ul style={{ margin: "0.5rem 0 0", paddingLeft: "1.25rem" }}>
      {files?.map((f) => {
        const url = urls?.files.find((u) => u.tableName === f.tableName)?.url;
        return (
          <li key={f._id}>
...
function App() {
  ...
  return (
    <>
      ...
      <div className="card">
        <div style={box}>
          <button onClick={() => void seed({ users: 5 })}>Seed sample data</button>{" "}
          <input
            value={tables}
          ...
          />{" "}
          <button onClick={handleRun}>Run export</button>
        </div>
