...
| [`derived-datasets-design.md`](./derived-datasets-design.md)                   | Design brainstorm for Tableau-like transformation over the catalog: transform specs → virtual derived datasets (lookup + rollup); the Projects-layer question is assessed and deferred in §9 (superseded in part — see the lifecycle doc). Design only — not implemented. |
| [`catalog-lifecycle-design.md`](./catalog-lifecycle-design.md)                 | Draft → publish lifecycle: projects as the virtual working layer, publish materializes + freezes into the catalog, versioned consumption (notify/diff/sync/revert) on the existing frozen-version machinery. Companion to the derived-datasets doc; ADR 0008. Design only — not implemented. |
| [`analysis-layer-design.md`](./analysis-layer-design.md)                       | Future design for SQL analytics over catalog/project data (DuckDB-WASM, client-side): the invariants to protect now (one row-resolution seam, typed structures, serializable specs, published immutability) and the later Parquet-sidecar hook. Design futures — not implemented. |
| [`data-platform-roadmap.md`](./data-platform-roadmap.md)                       | Umbrella long-term plan for the whole arc: Phase 0 structural preparation (auth gating, row-resolution seam, generalized version-freezing, coercion utilities, CI), then implementation stages 1–9 in dependency order with value checkpoints and the issue-filing convention. |
