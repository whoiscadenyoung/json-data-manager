...
import { flashCellKey, useDataTable, useDataTableActiveCell } from "./data-table-context";
...
const measureRowWithExpansion:
  | ((element: Element, entry?: ResizeObserverEntry) => number)
  | undefined =
  typeof window !== "undefined" && navigator.userAgent.indexOf("Firefox") === -1
    ? (element, entry) => {
        ...
        if (next && next.getAttribute("data-slot") === "datatable-expanded-row") {
...
export const DataTableVirtualizedHeader = React.memo(function DataTableVirtualizedHeader({
  className,
  sticky = true,
      ...
      {headerGroups.map((headerGroup) => (
        <TableRow key={headerGroup.id}>
          {headerGroup.headers.map((header) => {
            ...
            const isActiveColumn = activeCell?.columnIds.has(header.column.id) ?? false;
            return (
              <TableHead
                key={header.id}
                ...
                className={cn(
                  ...
                  isActiveColumn && "bg-primary/15! font-semibold! text-foreground!",
 ...
 */
export const DataTableVirtualizedFlexHeader = React.memo(function DataTableVirtualizedFlexHeader({
  className,
  sticky = true,
      ...
      {headerGroups.map((headerGroup) => (
        <TableRow key={headerGroup.id} className="flex w-full border-b">
          {headerGroup.headers.map((header) => {
            ...
            const fixedWidth = resizing ? header.getSize() : size ? `${size}px` : undefined;
            return (
              <TableHead
                key={header.id}
                data-col-id={header.column.id}
...
const VirtualizedBodyRowInner = function VirtualizedBodyRow<TData extends RowData>({
  row,
  virtualIndex,
  expandColumnId,
  isExpanded,
  ...
  return (
    <>
      {menuItems ? (
        <DataTableRowContextMenu row={row.original as TData} trigger={rowElement}>
          {menuItems}
...
const VirtualizedBodyRow = React.memo(VirtualizedBodyRowInner) as typeof VirtualizedBodyRowInner;
...
export function DataTableVirtualizedBody<TData extends RowData>({
  ...
  const { table, columns, registerRowScroller, flashingRowIds, flashingCellKeys, headerMinWidths } =
    useDataTable();
  ...
  const parentRef = React.useCallback((node: HTMLTableSectionElement | null) => {
    tbodyRef.current = node;
    if (node !== null) {
      const container = node.closest('[data-slot="table-container"]') as HTMLDivElement | null;
  ...
  React.useLayoutEffect(() => {
    ...
    if (columnLockRef.current && lockedColumnCountRef.current !== currentColCount) {
      columnLockRef.current = false;
      setColumnsLocked(false);
    ...
    const ths = tableEl.querySelectorAll<HTMLTableCellElement>("thead [data-slot='table-head']");
    ...
    const totalDesiredWidth = leafColumns.reduce((sum, col) => sum + col.getSize(), 0);
    tableEl.style.minWidth = `${totalDesiredWidth}px`;
    ...
    const scale = containerWidth > 0 && effectiveSum > 0 ? containerWidth / effectiveSum : 1;
  ...
  }, [scrollElement, onScroll, onScrolledTop, onScrolledBottom, scrollThreshold]);
  ...
  const lastItem = hasVirtualItems ? virtualItems[virtualItems.length - 1]! : null;
  const bottomSpacerHeight = lastItem ? rowVirtualizer.getTotalSize() - lastItem.end : 0;
  ...
  const stableMeasureElement = React.useCallback((node: HTMLTableRowElement | null) => {
      ...
      {virtualItems.map((virtualRow) => {
        ...
        return (
          <VirtualizedBodyRow
            ...
            measureRef={
              fixedRowHeight ? undefined : columnsLocked ? stableMeasureElement : undefined
            }
...
export function DataTableVirtualizedSkeleton({
  ...
  if (children) {
    return (
      <TableRow>
        <TableCell colSpan={visibleColumns.length} className={cn("h-24 text-center", className)}>
          {children}
        </TableCell>
      </TableRow>
    );
      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <TableRow key={rowIndex} style={{ height: `${estimateSize}px` }}>
          {visibleColumns.map((column, colIndex) => {
            ...
            return (
              <TableCell key={colIndex} className={cn(cellClassName)} style={cellStyle}>
